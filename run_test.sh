...
for i in $(seq 1 $num_runs); do
    ...
    python train_v3.py --test-mode --num-episodes $num_episodes --load-dir $loadingdir --exp-name test_slow_$i  &
done
...
for i in $(seq 1 $num_runs); do
    ...
    python train_v3.py --test-mode --num-episodes $num_episodes --agent-limitation test_stuck --restore --load-dir $loadingdir --exp-name test_stuck_$i  &
done


for i in $(seq 1 $num_runs); do
    loadingdir=./saved_policy/2-agent/WPM/fully-connected/stuck_$i/
    python train_v3.py --num-agents 2 --num-landmarks 2 --rsrn-type WPM --network fully-connected --num-episodes $num_episodes --agent-limitation test_stuck --restore --load-dir $loadingdir --exp-name test_stuck_$i  &
done
